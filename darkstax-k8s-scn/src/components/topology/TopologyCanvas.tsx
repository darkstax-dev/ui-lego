...
export function TopologyCanvas() {
  const { filters, openMetadataPanel, setSelectedNode } = useUIStore();
  ...
  useEffect(() => {
    ...
    // Build groups from hierarchy configuration rules
    const generatedGroups = buildGroupsFromRules(
      kubernetesTopologyScenario.nodes,
      hierarchyConfig.groupingRules
    );
    
    // Use generated groups if available, otherwise fall back to scenario groups
    if (generatedGroups.length > 0) {
      setGroups(generatedGroups);
    } else if (kubernetesTopologyScenario.groups) {
      setGroups(kubernetesTopologyScenario.groups);
    }
  }, [setNodes, setGroups]);
  ...
  // Load lane categories from hierarchy configuration
  ...
  const computeConnections = () => {
    ...
    const renderedIds = new Set(renderedNodes.map((n) => n.id));
      ...
      <div className="w-full min-h-full bg-gray-300 relative p-5 pt-5">
        ...
        >
          <defs>
            ...
            >
              <path d="M 0 0 L 10 5 L 0 10 z" fill="#00112B" opacity="0.55" />
            </marker>
          </defs>
