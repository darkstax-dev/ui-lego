...
export class Topology extends React.Component<Props, {}> {
    ...
    private showNodeContextMenu(event: any, d: D3Node) {
        if (!this.svgDiv) {
            return
        }
    ...
    private nodeClicked(event: any, d: D3Node) {
    ...
    private nodeDoubleClicked(event: any, d: D3Node) {
        event.stopPropagation()
    ...
    private linkClicked(event: any, d: Link) {
        event.stopPropagation()
    ...
    private renderLevels() {
        ...
        levelEnter.transition()
            .duration(animDuration)
            .style("opacity", 1)
            .on('end', (_event: any, d: LevelRect) => this.showLevelLabel(d))
    ...
    private renderHieraLinks(root: any) {
        ...
        hieraLink.transition().attr("d", hieraLinker)
    }
    ...
    private renderGroups() {
        ...
        const handleBraces = (g: any, d: NodeWrapper, animated: boolean) => {
            ...
            if (animated) {
                brace = brace.transition().duration(animDuration)
            }
        ...
        groupEnter.append("path").attr("class", "group-brace-bg")
        groupEnter.append("path").attr("class", "group-brace-owner-bg").style("opacity", 1)
        groupEnter.append("path").attr("class", "group-brace group-brace-left")
        groupEnter.append("path").attr("class", "group-brace group-brace-right")
        ...
        const handleIcon = (gIcon: any, d: NodeWrapper, dy: number, disabled: boolean) => {
            ...
            gIcon = gIcon.transition().duration(animDuration)
            ...
            gIcon
                .style("opacity", opacity)
                .attr("transform", (_d: D3Node) => d3node ? `translate(${d3node.x + nodeWidth / 2},${y + dy})` : ``)
        }
        ...
        leftIcon.append("text")
            .text("\uf191")
            .on("click", (event: any, d: NodeWrapper) => {
                event.stopPropagation()
                if (!d.wrapped.state.expanded) {
                    return
                }
        ...
        fullIcon.append("text")
            .text("\uf0fe")
            .on("click", function (event: any, d: NodeWrapper) {
                event.stopPropagation()
    ...
    private renderNodes(root: any) {
        ...
        var nodeEnter = node.enter()
            ...
            .on("dblclick", (event: any, d: D3Node) => this.nodeDoubleClicked(event, d))
            .on("click", (event: any, d: D3Node) => this.nodeClicked(event, d))
            .on("contextmenu", (event: any, d: D3Node) => {
                event.preventDefault()
                this.showNodeContextMenu(event, d)
            })
            .on("mouseover", (_event: any, d: D3Node) => {
                this.overNode(d.data.id, true)
            })
            .on("mouseout", (_event: any, d: D3Node) => {
    ...
    private renderLinks() {
        ...
        var linkWrapEnter = linkWrap.enter()
            ...
            .on("click", (event: any, d: Link) => this.linkClicked(event, d))
            .on("mouseover", (_event: any, d: Link) => {
                if (isVisible(d)) {
                    select("#link-overlay-" + d.id)
                        .style("opacity", 1)
                }
            })
            .on("mouseout", (_event: any, d: Link) => {
