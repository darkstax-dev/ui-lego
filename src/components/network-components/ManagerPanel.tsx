...
export const ManagerPanel: React.FC<ManagerPanelProps> = ({
  ...
  const renderTabContent = () => {
    const accordionContent = (layerId: string, layerName: string) => (
      <div className="manager-panel__accordion-content">
        <p style={{ padding: '12px', margin: 0, color: '#78797A', fontSize: '14px' }}>
          {layerName} content goes here
        </p>
      </div>
    )

    const accordionTitle = (layerName: string, layerId: string, visible: boolean) => (
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', width: '100%' }}>
        <button
          className="manager-panel__accordion-eye"
          onClick={(e) => {
            e.stopPropagation()
    ...
    switch (currentTab) {
      case 'layout':
        return (
          <div className="manager-panel__tab-content">
            <AccordionItem
              title={accordionTitle('Layout Settings', 'layout-1', true)}
              content={accordionContent('layout-1', 'Layout Settings')}
              defaultOpen={true}
            />
          </div>
        )
        ...
        return (
          <div className="manager-panel__tab-content">
            {layers.map((layer) => (
              <AccordionItem
                key={layer.id}
                title={accordionTitle(layer.name, layer.id, layer.visible)}
                content={accordionContent(layer.id, layer.name)}
                defaultOpen={layer.expanded}
              />
            ))}
          </div>
        )
      ...
      case 'view':
        return (
          <div className="manager-panel__tab-content">
            <AccordionItem
              title={accordionTitle('View Options', 'view-1', true)}
              content={accordionContent('view-1', 'View Options')}
              defaultOpen={true}
            />
          </div>
        )
      ...
      case 'tools':
        return (
          <div className="manager-panel__tab-content">
            <AccordionItem
              title={accordionTitle('Tool Settings', 'tools-1', true)}
              content={accordionContent('tools-1', 'Tool Settings')}
              defaultOpen={true}
            />
          </div>
        )
